...
static NanPersistent<v8::Function> callbackfunction;
void weakCallback(
  ...
  NanMakeCallback(NanGetCurrentContext()->Global(), NanNew(callbackfunction), 1, &val);
  delete parameter;
}
...
v8::Handle<v8::String> wrap() {
  ...
  persistent.SetWeak(static_cast<int*>(0), weakCallback, NanWeakCallbackType::kInternalFields);
  assert(persistent.IsWeak());
  return scope.Escape(lstring);
}

NAN_METHOD(Hustle) {
  callbackfunction.Reset(args[0].As<v8::Function>());
  NanReturnValue(wrap());
}
